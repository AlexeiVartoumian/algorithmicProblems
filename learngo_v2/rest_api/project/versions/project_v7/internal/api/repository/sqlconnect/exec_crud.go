...
func AddExecsDBHandler(newExecs []models.Exec) ([]models.Exec, error) {
	...
	for i, newExec := range newExecs {
		...
		if newExec.Password == "" {
			return nil, utils.ErrorHandler(errors.New("please enter password"), "please enter password")
		}
		salt := make([]byte, 16)
		_, err := rand.Read(salt)
		if err != nil {
			return nil, utils.ErrorHandler(errors.New("Failed to generate salt"), "error adding the datums")
		}
		//save hashing
